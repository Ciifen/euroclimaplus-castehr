...
Modification
Darwin Rosero
add new dataset and integrate with local stations
"""

import math

from eto._check import (
...
def penman_monteith(net_rad, t, ws, svp, avp, delta_svp, psy, shf=0.0):
    """
...
def sunset_hour_angle(latitude, sol_dec):
    ...
    :rtype: float
    """
    ##print("sunset_hour_angle:latitude : ",latitude," --")
    _check_latitude_rad(latitude)
    _check_sol_dec_rad(sol_dec)
...
def penman_monteith_daily_rtvh(tmin,tmax, tmed ,ws,hf,day_of_year,psychrometer,latitude,altitude, albedo,shf = 0.0):
    """Estimate de etp with a tipic data in meteorological centers from south america
    this base on de basic parameter messure in de stations, to simplify the estimatiosn

    """
    _sol_dec = sol_dec(day_of_year)
    _sha = sunset_hour_angle(convert.deg2rad(latitude), _sol_dec)
    _dlh= daylight_hours(_sha)
    _et_rad = et_rad(convert.deg2rad(latitude),_sol_dec,_sha,inv_rel_dist_earth_sun(day_of_year))
    _sol_rad = sol_rad_from_sun_hours(_dlh, hf, _et_rad)
    _ni_sw_rad = net_in_sol_rad(_sol_rad,albedo=albedo)
    _cs_rad=cs_rad(altitude, _et_rad)
    _avp = avp_from_tmin(tmin)
    _no_lw_rad = net_out_lw_rad(convert.celsius2kelvin(tmin), convert.celsius2kelvin(tmax), _sol_rad, _cs_rad, _avp)
    _net_rad = net_rad(_ni_sw_rad,_no_lw_rad)
    _svp = svp_from_t(tmed)
    _delta_svp = delta_svp(tmed)
    _atmos_pres = atm_pressure(altitude)
    _psy = psy_const_of_psychrometer(psychrometer, _atmos_pres)
    return penman_monteith(_net_rad,convert.celsius2kelvin(tmed),ws,_svp,_avp,_delta_svp,_psy,shf)

def penman_monteith_mounthly_rtvh(tmin,tmax, tmed ,ws,hf,day_of_year,psychrometer,latitude,altitude, albedo,shf = 0.0):
    """Estimate de etp with a tipic data in meteorological centers from south america
    this base on de basic parameter messure in de stations, to simplify the estimatiosn
...
def hargraves_daily_rtvh(tmin,tmax,tmean,latitude,day_of_year):
    _sol_dec = sol_dec(day_of_year)
    _sha = sunset_hour_angle(convert.deg2rad(latitude), _sol_dec)
    _et_rad = et_rad(convert.deg2rad(latitude), _sol_dec, _sha, inv_rel_dist_earth_sun(day_of_year))
    return hargreaves(tmin,tmax,tmean,_et_rad)
