...
import calendar
import eto.computeETP as pennman
from eto._check import check_latitude_rad as _check_latitude_rad
...
def monthly_mean_daylight_hours(latitude, year=None):
    ...
    for mdays in month_days:
        ...
        for daynum in range(1, mdays + 1):
            sd = pennman.sol_dec(doy)
            sha = pennman.sunset_hour_angle(latitude, sd)
            dlh += pennman.daylight_hours(sha)
            doy += 1
